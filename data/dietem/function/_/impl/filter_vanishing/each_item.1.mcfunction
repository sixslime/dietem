...
kill @s
execute as @e[type=item,distance=0..1] if items entity @s container.0 *[minecraft:custom_data~{dietem:{_:{vanish_check:true}}}] run function dietem:_/impl/filter_vanishing/item
data remove storage dietem:out filter_vanishing.result[-1].components."minecraft:custom_data".dietem